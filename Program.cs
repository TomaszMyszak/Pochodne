...
namespace Pochodna
{
    class Program
    {
        static double pochodna(Func<double, double> f, double x, double dx)
        {
        ...
        static void testuj (double x, double dx = 0.0001)
        {
            ...
            double y1 = pochodna(f, x, dx);
            Console.WriteLine("Wartość funkcji w x = " + x + ": " + y1);
            double y2 = pochodna(f, x, dx);
            Console.WriteLine("Wartość funkcji w x = " + x + ": " + y2);
        }
